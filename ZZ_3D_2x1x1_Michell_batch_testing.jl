...
function analyze_tet_mesh(filepath::String, taskname::String, output_dir::String)
    ...
    # Export combined results (displacement + stress + BC) to single VTU
    let out = joinpath(output_dir, "$(taskname)_results")
        ncells = getncells(grid)
        vm = zeros(ncells)
        σ_max = zeros(ncells)
        σ_min = zeros(ncells)
        for (cid, stensors) in stress_field
            isempty(stensors) && continue
            avg = sum(stensors) / length(stensors)
            vm[cid] = sqrt(3/2 * (dev(avg) ⊡ dev(avg)))
            eigs = sort(LinearAlgebra.eigvals(avg))
            σ_max[cid] = eigs[3]
            σ_min[cid] = eigs[1]
        end
        bc_marker = zeros(Int, getnnodes(grid))
        for nid in support_left
            ;
            bc_marker[nid] = 1;
        end
        for nid in support_right
            ;
            bc_marker[nid] = 1;
        end
        for nid in force_nodes
            ;
            bc_marker[nid] = 2;
        end

        VTKGridFile(out, dh) do vtk
            write_solution(vtk, dh, u)
            write_cell_data(vtk, vm, "von_Mises_stress")
            write_cell_data(vtk, σ_max, "principal_stress_max")
            write_cell_data(vtk, σ_min, "principal_stress_min")
            write_node_data(vtk, bc_marker, "boundary_type")
        end
        print_info("Results exported to $(out).vtu")
    end
